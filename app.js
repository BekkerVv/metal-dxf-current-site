...
async function addLayoutFiles(files){
  const list=[...files].filter(f=>/^image\//.test(f.type)||/\.(png|jpe?g)$/i.test(f.name));
  for(const file of list){ const imageData=await fileToDataUrl(file); state.layouts.push({id:state.nextLayoutId++, fileName:file.name, imageData, material:getSettings().defaultMaterial, thickness:getSettings().defaultThickness, sheetSize:'1500×3000', sheetCount:1, usage:'', fill:'', comment:''}); }
  renderAll(); autosaveProject();
}
